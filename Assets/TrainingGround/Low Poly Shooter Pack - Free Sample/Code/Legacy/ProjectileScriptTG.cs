...
public class ProjectileScriptTG : MonoBehaviour {
	...
	private void OnCollisionEnter (Collision collision)
	{
		...
		if (collision.gameObject.tag == "Target" && 
		    	collision.gameObject.GetComponent<TargetScriptTG>().isHit == false) {
		...
		foreach (Collider hit in colliders)
		{
			...
			if (hit.GetComponent<Collider>().tag == "Target" && 
			    	hit.GetComponent<TargetScriptTG>().isHit == false) {

				//Toggle the isHit bool on the target object
				hit.gameObject.GetComponent<TargetScriptTG>().isHit = true;
			}
			...
			if (hit.transform.tag == "ExplosiveBarrel") {
				...
				hit.transform.gameObject.GetComponent<ExplosiveBarrelScriptTG>().explode = true;
			}
			...
			if (hit.GetComponent<Collider>().tag == "GasTank") 
			{
				...
				hit.gameObject.GetComponent<GasTankScriptTG> ().isHit = true;
				hit.gameObject.GetComponent<GasTankScriptTG> ().explosionTimer = 0.05f;
			}
		}
	}
}